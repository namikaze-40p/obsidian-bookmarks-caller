...
export class MessageModal extends Modal {
  ...
  onOpen(): void {
    ...
    notionEl
      .createSpan()
      .setText('This plugin needs bookmarks plugin, but bookmarks plugin is disabled.');
    notionEl
      .createSpan()
      .setText('You can enable bookmarks plugin under `Settings` → `Core plugins` → `Bookmarks`.');
